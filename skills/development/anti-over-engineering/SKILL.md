...
## Grinding vs Anti-Over-Engineering
...
- This skill can cause under-delivery if applied to tasks where the user expects proactive improvement. Use judgement: when the user's implicit expectation is production quality, a little proactive polish (e.g., fixing a glaring typo in a related error message) is acceptable.
- The 4-level detection system is a guide, not a script. Real over-engineering is a spectrum; use the levels as reference points, not hard rules.
- Pairing with a "PUA" (push forward) skill is recommended for agents that naturally tend toward under-engineering. This skill sets the ceiling; PUA sets the floor.

## Verification

To verify this skill is active and working:
1. Give the agent a scoped task (e.g., "fix the import error in line 42 of app.py")
2. After completion, check: did the agent modify only the specified file and lines?
3. Request an unnecessarily complex approach (e.g., "add error handling to this function") and verify the agent flags it as outside scope unless explicitly asked.
4. Run the checklist from this skill and confirm all items pass.